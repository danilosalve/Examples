...
User function DSTREE()
	...
	TButton():New( 160, 152, "Novo Item"	, oDlg,{|| TreeNewIt(oTree) },40,010,,,.F.,.T.,.F.,,.F.,,,.F. )    
	TButton():New( 172,02,"Dados do item"	, oDlg,{|| Alert("Cargo: "+oTree:GetCargo()+chr(13)+"Texto: "+oTree:GetPrompt(.T.)) },;
	40,10,,,.F.,.T.,.F.,,.F.,,,.F. )    
	TButton():New( 172, 052, "Muda Texto"	, oDlg,{||	oTree:ChangePrompt("Novo Texto Item 001","001") },;
	40,010,,,.F.,.T.,.F.,,.F.,,,.F. )    
	TButton():New( 172, 102, "Muda Imagem"	, oDlg,{||oTree:ChangeBmp("LBNO","LBTIK",,,"001") },;
	40,010,,,.F.,.T.,.F.,,.F.,,,.F. )    
	TButton():New( 172, 152, "Apaga Item"	, oDlg,{||	if(oTree:TreeSeek("006"),oTree:DelItem(),) },;
...
Static Function TreeNewIt(oTree)  
